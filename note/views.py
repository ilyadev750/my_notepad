...
from .functions import prepare_data_for_form, add_info_in_new_object_and_session, add_info_in_session, \
    extract_data_from_object, add_info_in_current_object_and_session, make_pdf
...
def anonymous_note(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if 'register/login' in request.POST: # separate login and register
...
def delete_user_note(request, *args, **kwargs):
    if request.user.is_authenticated:
        current_user_object = Note.objects.get(id=kwargs['note_id'], title=kwargs['title'])
